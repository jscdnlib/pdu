...
****/
    ...
    function tokenizer( octets ) {
        ...
        if (pduType.type === 'deliver') {
            ...
            dataList.dcs = TP_DCS.info; 

            pos++;
            var sliceTimeStamp = octets.slice( pos, pos + 7 );
            dataList.smsc_time = tokens.SCTS(sliceTimeStamp); 
            pos += 6;
        }
        else if (pduType.type === 'submit') {
            pos++;
            var MR = octets[ pos ];
            dataList.messref = tokens.MR(MR); 
            pos++;
            numberLength = parseInt(octets[pos], 16 );
            pos++;
            if (numberLength) {
            ...
            if (pduType.TP_VPF) {
                ...
                if (pduType.TP_VPF === 'relative') {
                    sliceVP = octets[ pos ];
		            dataList.valpri = tokens.VPrelative(sliceVP); 
                }
                else if (pduType.TP_VPF.match( /^(absolute|relative)$/ )) {
                    sliceVP = octets.slice( pos, pos + 7 );
		            dataList.valpriuntil = tokens.SCTS(sliceVP); 
                    pos += 6;
                }
            }
        }
        ...
        dataList.sms_length = TP_UDL.info; 
        ...
        if (pduType.TP_UDHI) {
            ...
            dataList.userDataHeaderLength = TP_UDHL.info; 

            pos++;
            TP_UDH = tokens.UDH( octets.slice( pos, pos + TP_UDHL.length ) );
            dataList.userDataHeader = TP_UDH.info; 
